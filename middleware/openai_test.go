...
func TestVideoGenerationsMiddleware(t *testing.T) {
	type testCase struct {
		name string
		body string
		req  api.GenerateRequest
	...
	testCases := []testCase{
		{
			name: "video generation basic leaves output format unset",
			body: `{
				"model": "test-model",
		...
		{
			name: "video generation with explicit webm output format",
			body: `{
				"model": "test-model",
				"prompt": "a cat playing",
				"output_format": "webm"
		...
		{
			name: "video generation with explicit output format",
			body: `{
				"model": "test-model",
				"prompt": "a cat playing",
				"output_format": "gif"
		...
		{
			name: "video generation with size and frames",
			body: `{
				"model": "test-model",
				"prompt": "a cat playing",
				"size": "832x480",
		...
		{
			name: "video generation missing prompt",
			body: `{
				"model": "test-model"
			}`,
			err: openai.ErrorResponse{
		...
		{
			name: "video generation missing model",
			body: `{
				"prompt": "a cat playing"
			}`,
			err: openai.ErrorResponse{
	...
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.Use(VideoGenerationsMiddleware(), captureRequestMiddleware(&capturedRequest))
	router.Handle(http.MethodPost, "/api/generate", endpoint)
...
func TestVideoWriterResponse(t *testing.T) {
	gin.SetMode(gin.TestMode)

	t.Run("real webm container takes priority over frame images", func(t *testing.T) {
		// Simulates the diffgen runner streaming per-frame progress images
		// followed by a final Done message carrying a real webm container
		// (see x/diffgen EncodeWebM). The container must win, not the last
		// streamed frame image.
		endpoint := func(c *gin.Context) {
			enc := json.NewEncoder(c.Writer)
			enc.Encode(api.GenerateResponse{Model: "test-model", Image: "ZnJhbWUtMQ=="})
			enc.Encode(api.GenerateResponse{Model: "test-model", Image: "ZnJhbWUtMg=="})
			enc.Encode(api.GenerateResponse{
				Model:     "test-model",
				CreatedAt: time.Unix(1234567890, 0).UTC(),
				Done:      true,
				Video:     "d2VibS1jb250YWluZXItYnl0ZXM=", // base64 of "webm-container-bytes"
			})
		}

		router := gin.New()
		router.Use(VideoGenerationsMiddleware())
		router.Handle(http.MethodPost, "/api/generate", endpoint)
		...
		var videoResp openai.VideoGenerationResponse
		if err := json.Unmarshal(resp.Body.Bytes(), &videoResp); err != nil {
			t.Fatalf("failed to unmarshal response: %v", err)
		}
		if len(videoResp.Data) != 1 {
			t.Fatalf("expected 1 video, got %d", len(videoResp.Data))
		}
		if videoResp.Data[0].B64JSON != "d2VibS1jb250YWluZXItYnl0ZXM=" {
			t.Errorf("expected the webm container, got %s", videoResp.Data[0].B64JSON)
		}
		if videoResp.Data[0].Format != "webm" {
			t.Errorf("expected format 'webm', got %q", videoResp.Data[0].Format)
		}
	})

	t.Run("falls back to last frame image when no container is produced", func(t *testing.T) {
		// Simulates the legacy frame-stream protocol (no container encoder
		// available): only per-frame images are streamed, and the final
		// Done message carries no Video.
		endpoint := func(c *gin.Context) {
			enc := json.NewEncoder(c.Writer)
			enc.Encode(api.GenerateResponse{Model: "test-model", Image: "ZnJhbWUtMQ=="})
			enc.Encode(api.GenerateResponse{Model: "test-model", Image: "ZnJhbWUtMg=="})
		...
		if len(videoResp.Data) != 1 {
			t.Fatalf("expected 1 entry, got %d", len(videoResp.Data))
		}
		if videoResp.Data[0].B64JSON != "ZnJhbWUtMg==" {
			t.Errorf("expected the last streamed frame, got %s", videoResp.Data[0].B64JSON)
		}
		if videoResp.Data[0].Format != "png" {
			t.Errorf("expected format 'png', got %q", videoResp.Data[0].Format)
		}
	})
}

func TestVideoEditsMiddleware(t *testing.T) {
	type testCase struct {
		name string
		body string
		req  api.GenerateRequest
	...
	testCases := []testCase{
		{
			name: "video edit basic leaves output format unset",
			body: `{
				"model": "test-model",
				"prompt": "animate this",
				"image": "` + testImage + `"
		...
		{
			name: "video edit with explicit webm output format",
			body: `{
				"model": "test-model",
				"prompt": "animate this",
				"image": "` + testImage + `",
		...
		{
			name: "video edit missing prompt",
			body: `{
				"model": "test-model",
				"image": "` + testImage + `"
			}`,
	...
	gin.SetMode(gin.TestMode)
	router := gin.New()
	router.Use(VideoEditsMiddleware(), captureRequestMiddleware(&capturedRequest))
	router.Handle(http.MethodPost, "/api/generate", endpoint)
